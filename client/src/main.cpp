...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	if(settings.iConsole)
	{
		Log(" ");
		Log("* ===================================================== *");
		Log("  Hack iniciado %02d/%02d/%02d %02d:%02d:%02d",
	...
	while(1)
	{
		...
		if (iAreWeConnected && iGameInited)
		{
			...
			if(dwLastInfoUpdate && dwLastInfoUpdate < (GetTickCount() - 1000))
			{
				...
				sprintf_s(szInfo, 400, "Server: %s\nPlayers NoNPC: %d\tTotal: %d\tPing: %d\tVida: %s     Armadura: %s     Skin: %d     X: %.4f     Y: %.4f     Z: %.4f     Rotacion: %.4f",
				g_szHostName, getPlayerCountWoNPC(), getPlayerCount(), playerInfo[g_myPlayerID].dwPing, szHealthText, szArmourText, iLocalPlayerSkin, settings.fNormalModePos[0], settings.fNormalModePos[1], settings.fNormalModePos[2], settings.fNormalModeRot);
