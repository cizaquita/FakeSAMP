...
void InitGame(RPCParameters *rpcParams)
{
	...
	{
		sprintf(szTitle, "%s (%d) - %.16s - Owned 8)", g_szNickName, g_myPlayerID, g_szHostName);
		SetConsoleTitle(szTitle);
		Log("Connected to %.64s\n", g_szHostName);
	}
	else
	{
		sprintf(szTitle, "%s (%d) - Owned 8)", g_szNickName, g_myPlayerID);
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	else if(byteRejectReason==REJECT_REASON_BAD_NICKNAME)
	{
		...
		Log("[SAMP] Bad nickname. Changing name to %s", szNewNick);
	...
	if(settings.iFind)
	{
		for(int i = 0; i < MAX_FIND_ITEMS; i++)
		{
			if(!settings.findItems[i].iExists)
				continue;
			//If Say TEXT have ! perform Client Command
			//If Say TEXT have / perform SererCommand
			//Added Sleep function to wait some second then execute next line
			if(strstr(szMsg, settings.findItems[i].szFind)){
				if(settings.findItems[i].szSay[0] != 0x00){
					Sleep(5000);
					if(settings.findItems[i].szSay[0] == '/')
						sendServerCommand(settings.findItems[i].szSay);
					else if (settings.findItems[i].szSay[0] == '!')
						RunCommand(settings.findItems[i].szSay, 1);
					else
