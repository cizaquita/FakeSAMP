...
int RunCommand(char *szCMD, int iFromAutorun)
{
	...
	if(!strncmp(szCMD, "diares", 6) || !strncmp(szCMD, "DIARES", 6))
	{
		int szDialogID, szButtonID, szListBoxItem;
		char szInputResp[128];

		if(sscanf(&szCMD[7], "%d%d%d%s", &szDialogID, &szButtonID, &szListBoxItem, szInputResp) < 4)
		{
			Log("USAGE: !diares <Dialog ID> <Button ID> <Listbox item> <Input response>");
			return 1;
		}

		sendDialogResponse(szDialogID, szButtonID, szListBoxItem, szInputResp);

		Log("Dialog response enviado.");
		return 1;
	}
